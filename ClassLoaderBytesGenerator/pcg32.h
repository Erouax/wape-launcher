...
struct pcg32 {
  ...
  /// Initialize the pseudorandom number generator with the ef seed() function
  explicit pcg32(uint64_t initstate, uint64_t initseq = 1u) {
  ...
  */
  ...
  uint32_t nextUInt() {
    ...
    auto xorshifted = static_cast<uint32_t>(
  ...
  *
  * emark Since the underlying random number generator produces 32 bit output,
  * only the first 32 mantissa bits will be filled (however, the resolution is still
  * finer than in ef nextFloat(), which only uses 23 mantissa bits)
  */
  double nextDouble() {
    /* Trick from MTGP: generate an uniformly distributed
  ...
  template <typename Iterator> void shuffle(Iterator begin, Iterator end) {
    for (Iterator it = end - 1; it > begin; --it) {
      std::iter_swap(it, begin +
        nextUInt(static_cast<uint32_t>(it - begin + 1)));
    }
  }
